...
{
    typedef uint32_t RHICommonFlags;
    typedef uint64_t RHIDeviceSize;
    typedef uint32_t RHIColorValue;

    typedef std::string RHIName;
    typedef std::string RHIPath;
    ...
    enum RHIFormat : int
    {
        ...
        // 浮点格式（高动态范围支持）
        RHI_Format_FloatRGBA,
        RHI_Format_FloatR11G11B10,

        // 深度和模板格式
        RHI_Format_DepthStencil,
        RHI_Format_ShadowDepth,
        RHI_Format_D16,
        RHI_Format_D32,
        RHI_Format_D24S8,

        // 高精度颜色格式
        RHI_Format_A2B10G10R10,
    ...
    enum RHIRTLoadActions {
        Load_NoAction,    // 不加载现有内容，通常会忽略或保持目标内容
        Load_Load,        // 加载现有内容，用于增量更新或继续操作
        Load_Clear        // 清除目标内容（通常用清除颜色或深度）
    };
    enum RHIRTStoreActions {
        Store_NoAction,    // 不存储渲染内容，目标数据可能被丢弃。
        Store_Store,       // 存储渲染内容，以便后续读取或处理。
        Store_MultisampleResolve // 将多采样数据解析到单采样缓冲区（MSAA 相关）。
    };
    enum RHIPipelineState {
        PipelineStage_None,
        PipelineStage_GRAPHICS,
        PipelineStage_COMPUTE,
        PipelineStage_RAY_TRACING_KHR,
        PipelineStage_SUBPASS_SHADING_HUAWEI,
        PipelineStage_RAY_TRACING_NV,
        PipelineStage_MAX_ENUM
    };
    enum RHIShderType {
        ShaderType_None,
        ShaderType_Vert,
