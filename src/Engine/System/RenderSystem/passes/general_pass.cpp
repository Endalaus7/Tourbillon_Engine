...
void TourBillon::GeneralPass::beforeDraw(float dt, RHIDrawInfo& drawinfo)
{
	...
	const auto& geo_components = ECSManager::Instance()->GetComponentEntities<GeometryPtr>();
	//drawinfo.drawMeshinfos.resize(geo_components.size());
	uint32_t meshindex = 0;
	uint32_t entityindex = 0;
	std::unordered_map<GeometryData*, uint32_t> loadRecord;
	for (auto& entity : geo_components)
	{
		auto& meshptr = ECSManager::Instance()->GetComponent<GeometryPtr>(entity);

		GeometryData* geometry = dynamic_cast<GeometryData*>(meshptr.getData());
